...
from util.augmentor import SequenceAugmentor
import sys
...
class UnisRec():
    def __init__(self, args, data):
        ...
        self.head_num = int(8)
        self.model = UniSRec_Module(self.data, self.args.emb_size, self.max_len, self.block_num, self.head_num, self.drop_rate, self.temperature,
                                    self.feature, self.args.data_path, self.args.dataset)
        self.rec_loss = torch.nn.BCEWithLogitsLoss()

        self.cl_rate_2=0.001
        self.aug_rate = 0.1
        self.cl_rate = 0.001
    ...
    def pretrain(self):
        ...
        for epoch in range(self.args.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_sequence(self.data, self.args.batch_size,max_len=self.max_len)):
                ...
                cl_loss_item = self.cl_rate * InfoNCE(torch.cat(cl_emb1, 0), torch.cat(cl_emb2, 0), 1, True)
                
                # cl_loss_item2 = self.cl_rate_2 * self.cal_cl_loss2(y_emb,pos)

                aug_seq1 = SequenceAugmentor.item_mask(seq, seq_len, self.aug_rate, self.data.item_num)
                aug_seq2 = SequenceAugmentor.item_mask(seq, seq_len, self.aug_rate, self.data.item_num)
                ...
                cl_loss_sentence = self.cl_rate_2 * InfoNCE(torch.cat(cl_emb1, 0), torch.cat(cl_emb2, 0), 1,True)

                rec_loss = self.calculate_loss(seq_emb, y, neg_idx, pos)
                # batch_loss = rec_loss
                batch_loss = cl_loss_item + cl_loss_sentence + rec_loss
    ...
    def save(self, stage):
        ...
        if stage == 'pretrain':
            torch.save(self.model.state_dict(), './model/'+self.args.dataset+'/UnisRec_pretrain.pt')
        elif stage == 'finetune':
            torch.save(self.model.state_dict(), './model/'+self.args.dataset+'/UnisRec_finetune.pt')
    ...
    def finetune(self, attack=None):
        ...
        for epoch in range(self.args.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_sequence(self.data, self.args.batch_size,max_len=self.max_len)):
                ...
                cl_loss_item = InfoNCE(torch.cat(cl_emb1, 0), torch.cat(cl_emb2, 0), 1)
                ...
                aug_emb1 = model.forward(aug_seq1, pos)
                aug_emb2 = model.forward(aug_seq2, pos)
                # print(aug_emb1.shape)
                aug_emb1 = model.moe_adaptor(aug_emb1)
                aug_emb2 = model.moe_adaptor(aug_emb2)
                cl_emb1 = [aug_emb1[i, last - 1, :].view(-1, self.args.emb_size) for i, last in enumerate(seq_len)]
                cl_emb2 = [aug_emb2[i, last - 1, :].view(-1, self.args.emb_size) for i, last in enumerate(seq_len)]
                cl_loss_sentence = self.cl_rate * InfoNCE(torch.cat(cl_emb1, 0), torch.cat(cl_emb2, 0), 1)

                rec_loss = self.calculate_loss(seq_emb, y, neg_idx, pos)
                batch_loss = rec_loss + cl_loss_item + cl_loss_sentence
                # Backward and optimize
                optimizer.zero_grad()
                batch_loss.backward()
                optimizer.step()
    ...
    def finetune(self, attack=None):
        if os.path.exists('./model/'+self.args.dataset+'/UnisRec_pretrain.pt'):
            state_dict = torch.load('./model/'+self.args.dataset+'/UnisRec_pretrain.pt')
            del state_dict["item_emb"]
            self.model = SASRec_Model(self.data, self.args.emb_size, self.max_len, self.block_num, self.head_num, self.drop_rate,
                                        self.feature, self.args.data_path, self.args.dataset)
            self.model.load_state_dict(state_dict, strict=False)
            print("Loaded the model checkpoint!")
...
class UniSRec_Module(nn.Module):
    def __init__(self, data, emb_size, max_len, n_blocks, n_heads, drop_rate, temperature, feature, data_path, dataset):
        super(UniSRec_Module, self).__init__()
