...
class ModalAttackMetric(object):
    """
    param 
    targetItem:list, targetItem: id
    """
    def __init__(self, rec_list, targetItem):
        self.rec_list = rec_list
        self.targetItem = targetItem
        # print("self.rec_list", self.rec_list)
        
    def hitRate(self):
        hit = 0
        for i in self.rec_list:
            for item in self.rec_list[i]:
                # print("item[0]", item[0])
                # print("(self.targetItem", self.targetItem)
                if item[0] in self.targetItem:
                    # print("hit up:", item[0], self.targetItem)
                    # print("i", i)
                    # print("break")
                    hit += 1
                    break  
        hitrate = hit/len(self.rec_list)
        return hitrate
        ...
        for i in self.rec_list:
            for item in self.rec_list[i]:
                ...
                if item[0] in self.targetItem:
                    ...
                    hit += 1
                    break
        recall = hit/len(self.rec_list)
        return recall
    
    def precision(self):
        hit = 0
        for i in self.rec_list:
            length = len(self.rec_list[i])
            for item in self.rec_list[i]:
                # print("item[0]", item[0])
                # print("(self.targetItem", self.targetItem)
                if item[0] in self.targetItem:
                    ...
                    hit += 1
                    break
        precision = hit/(len(self.rec_list)*length)
        return precision
    ...
    def NDCG(self):
        ...
        for i in self.rec_list:
            idcg = 0
            dcg = 0
            for rank, item in enumerate(self.rec_list[i]):
                # print("item[0]", item[0])
                # print("(self.targetItem", self.targetItem)
                if item[0] in self.targetItem:
                    # print("hit up:", item[0], self.targetItem)
                    # print("i", i)
                    # print("break")
                    dcg = 1 / np.log2(2 + rank)
                idcg += 1 / np.log2(2 + rank)
            ndcg += dcg/idcg
        ndcg = ndcg/len(self.rec_list)
        return ndcg
